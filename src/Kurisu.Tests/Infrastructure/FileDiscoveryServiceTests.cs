...
public sealed class FileDiscoveryServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-discovery-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(workspaceRoot, "docs", "private-notes.md"), "kurisu ignored");
