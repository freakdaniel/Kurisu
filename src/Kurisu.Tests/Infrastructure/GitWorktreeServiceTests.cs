...
public sealed class GitWorktreeServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-worktrees-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
        var managedWorktreePath = Path.Combine(homeRoot, ".kurisu", "worktrees", "session-123", "worktrees", "code-review");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-worktree-lifecycle-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
        ...
        try
        {
            ...
            var sessionConfigPath = Path.Combine(homeRoot, ".kurisu", "worktrees", "session-lifecycle", "config.json");
            ...
            Assert.False(Directory.Exists(Path.Combine(homeRoot, ".kurisu", "worktrees", "session-lifecycle")));
        }
        finally
        {
            if (Directory.Exists(root))
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-worktree-overlay-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
