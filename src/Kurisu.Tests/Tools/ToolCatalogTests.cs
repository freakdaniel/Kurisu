...
public sealed class ToolCatalogTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-tools-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            Assert.Contains(snapshot.Tools, tool => tool.Name == WellKnownToolNames.RunShellCommand && tool.ApprovalState == "deny" && !tool.IsEnabled);
            Assert.Contains(snapshot.Tools, tool => tool.Name == "read_file" && tool.SourcePath == "native://tools/read_file");
            Assert.Contains(snapshot.Tools, tool => tool.Name == WellKnownToolNames.TodoWrite && tool.SourcePath == "native://tools/todo_write");
            ...
            Assert.Contains(snapshot.Tools, tool => tool.Name == WellKnownToolNames.ExitPlanMode && tool.SourcePath == "native://tools/exit_plan_mode");
            Assert.Contains(snapshot.Tools, tool => tool.Name == WellKnownToolNames.WebFetch && tool.SourcePath == "native://tools/web_fetch");
            Assert.Contains(snapshot.Tools, tool => tool.Name == WellKnownToolNames.WebSearch && tool.SourcePath == "native://tools/web_search");
            ...
            Assert.Contains(snapshot.Tools, tool => tool.Name == WellKnownToolNames.AskUserQuestion && tool.SourcePath == "native://tools/ask_user_question");
