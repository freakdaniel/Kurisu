...
public sealed class NativeToolHostTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-native-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-native-scoped-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-shell-host-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-tool-search-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-task-tools-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-shell-timeout-host-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-webfetch-ssl-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-memory-tool-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            var projectResult = await host.ExecuteAsync(sourcePaths, new ExecuteNativeToolRequest
            {
                ...
                ApproveExecution = true,
                ArgumentsJson = """{"fact":"Remember that this project uses a desktop-native kurisu runtime","scope":"project"}"""
            });
            ...
            Assert.Contains("## Kurisu Added Memories", File.ReadAllText(projectMemoryPath));
            Assert.Contains("desktop-native kurisu runtime", File.ReadAllText(projectMemoryPath));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-todo-tool-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
            ...
            var result = await host.ExecuteAsync(sourcePaths, new ExecuteNativeToolRequest
            {
                ...
                ArgumentsJson =
                    """
                    {
                      ...
                      "todos":[
                        {"id":"todo-1","content":"Port kurisu core scheduling","status":"in_progress"},
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-plan-exit-tool-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-cron-tools-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-ask-user-tool-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-pretool-hook-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
...
{
    public bool IsSearchAvailable(KurisuRuntimeProfile runtimeProfile) => true;
