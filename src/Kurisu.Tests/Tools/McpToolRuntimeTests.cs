...
public sealed class McpToolRuntimeTests
{
    ...
    public async Task McpToolRuntimeService_DescribeAsync_ListsDiscoveredTools()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-runtime-{Guid.NewGuid():N}");
    ...
    public async Task NativeToolHostService_ExecuteAsync_AllowsReadOnlyMcpToolsWithoutManualApproval()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-readonly-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    public async Task NativeToolHostService_ExecuteAsync_GatesWritableMcpToolsUntilApproved()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-write-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    public async Task McpToolRuntimeService_ConnectServerAsync_SseTransport_DiscoversTools()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-sse-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    public async Task McpToolRuntimeService_ReadResourceAsync_ReturnsResourceContents()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-resource-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    public async Task McpToolRuntimeService_ListResourcesAsync_ReturnsResourceDefinitions()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-resource-list-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    public async Task NativeToolHostService_ExecuteAsync_McpClientPromptInvocation_ReturnsPromptMessages()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-prompt-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    public async Task NativeToolHostService_ExecuteAsync_McpClientResourceRead_DeniesUntrustedServer()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-resource-trust-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    public async Task McpToolRuntimeService_DescribeAsync_DeniesUntrustedWorkspaceWhenFolderTrustEnabled()
    {
        ...
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-workspace-trust-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            File.WriteAllText(
                Path.Combine(environment.HomeDirectory, ".kurisu", "settings.json"),
                """
                {
                  "security": {
                    "folderTrust": {
                ...
                """);
            File.WriteAllText(
                Path.Combine(environment.HomeDirectory, ".kurisu", "trustedFolders.json"),
    ...
    private static (WorkspacePaths Paths, FakeDesktopEnvironmentPaths Environment, KurisuRuntimeProfileService RuntimeProfileService, McpRegistryService Registry, McpToolRuntimeService Runtime) CreateRuntime(string root)
    {
