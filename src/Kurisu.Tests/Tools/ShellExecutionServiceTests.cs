...
public sealed class ShellExecutionServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-shell-service-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-shell-output-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-shell-timeout-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
