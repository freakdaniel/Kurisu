...
public sealed class LspToolServiceTests
{
    ...
    public async Task NativeToolHostService_ExecuteAsync_Lsp_UntrustedWorkspace_ReturnsTrustError()
    {
        ...
        try
        {
            var homeKurisuRoot = Path.Combine(fixture.Root, "home", ".kurisu");
            Directory.CreateDirectory(homeKurisuRoot);
            await File.WriteAllTextAsync(
                Path.Combine(homeKurisuRoot, "settings.json"),
                """
                {
                  "security": {
                    "folderTrust": {
                ...
                """);
            await File.WriteAllTextAsync(
                Path.Combine(homeKurisuRoot, "trustedFolders.json"),
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-lsp-tool-{Guid.NewGuid():N}");
