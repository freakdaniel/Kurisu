...
public sealed class AgentToolTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-agent-tool-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-agent-tool-error-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-agent-tool-events-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
