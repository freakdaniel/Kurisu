...
public sealed class WebToolServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-web-fetch-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-web-search-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            var handler = new StubHttpMessageHandler(async request =>
            {
                ...
                Assert.Contains("\"query\":\"kurisu code desktop\"", requestBody);
                ...
                return new HttpResponseMessage(HttpStatusCode.OK)
                {
                    Content = new StringContent(
                        """
                        {
                          "answer":"Kurisu Code Desktop continues the C# port with native tooling.",
                          "results":[
                            {
                              "title":"Port status",
                              "url":"https://example.com/port",
                              "content":"Desktop-native kurisu runtime progress",
                              "score":0.95,
            ...
            using var document = JsonDocument.Parse("""{"query":"kurisu code desktop"}""");
            var output = await service.SearchAsync(runtimeProfile, document.RootElement);

            Assert.Contains("Web search results for \"kurisu code desktop\" (via tavily):", output);
            Assert.Contains("Kurisu Code Desktop continues the C# port with native tooling.", output);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-web-search-user-settings-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-web-search-fallback-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-web-fetch-redirect-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-web-fetch-cross-redirect-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-web-host-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            var handler = new StubHttpMessageHandler(request =>
            {
                if (request.RequestUri!.Host.Equals("example.com", StringComparison.OrdinalIgnoreCase))
                {
                    return new HttpResponseMessage(HttpStatusCode.OK)
                    {
                        Content = new StringContent("<html><body><p>Kurisu desktop article body</p></body></html>", Encoding.UTF8, "text/html")
                    };
                }
            ...
            Assert.Contains("Kurisu desktop article body", fetchResult.Output);
