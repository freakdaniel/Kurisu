...
public sealed class SkillToolTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-skill-tool-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            var skillRoot = Path.Combine(workspaceRoot, ".kurisu", "skills", "code-review");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-skill-tool-error-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            var homeRoot = Path.Combine(root, "home");
            var systemRoot = Path.Combine(root, "system");
            var userSkillRoot = Path.Combine(homeRoot, ".kurisu", "skills", "testing");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-skill-tool-untrusted-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
