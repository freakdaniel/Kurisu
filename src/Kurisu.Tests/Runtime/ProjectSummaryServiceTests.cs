...
public sealed class ProjectSummaryServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-project-summary-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var summaryDirectory = Path.Combine(workspaceRoot, ".kurisu");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-project-summary-untrusted-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var summaryDirectory = Path.Combine(workspaceRoot, ".kurisu");
        Directory.CreateDirectory(summaryDirectory);
    ...
    private static KurisuRuntimeProfile CreateRuntimeProfile(string workspaceRoot, bool trusted) =>
        new()
        {
            ProjectRoot = workspaceRoot,
            GlobalKurisuDirectory = Path.Combine(Path.GetTempPath(), "kurisu-tests-home"),
            RuntimeBaseDirectory = Path.Combine(workspaceRoot, ".kurisu-runtime"),
            RuntimeSource = "test",
