...
public sealed class OpenAiCompatibleProviderTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-openai-stream-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-openai-stream-retry-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-openrouter-headers-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            Assert.Equal("https://kurisu.local", capturedRequest!.Headers.GetValues("HTTP-Referer").Single());
            Assert.Equal("Kurisu", capturedRequest.Headers.GetValues("X-OpenRouter-Title").Single());
        }
        finally
        {
            Directory.Delete(root, recursive: true);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-openai-system-prompt-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-modelscope-fallback-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-openai-http-retry-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    [Fact]
    public async Task OpenAiCompatibleAssistantResponseProvider_TryGenerateAsync_RetriesInsufficientQuota429OutsideKurisuOAuth()
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-openai-quota-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
