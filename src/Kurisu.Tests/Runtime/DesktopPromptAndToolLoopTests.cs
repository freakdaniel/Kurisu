...
public sealed class DesktopPromptAndToolLoopTests
{
    ...
    public void OpenAiCompatibleProtocol_BuildPayload_UsesAgenticCurrentTurnPrompt()
    {
        ...
        var promptContext = new AssistantPromptContext
        {
            ...
            UserInstructionSummary = "From ~/.kurisu/KURISU.md:\n- Prefer native runtime integrations.",
            WorkspaceInstructionSummary = "From KURISU.md:\n- Keep reconnect flow stable.",
            DurableMemorySummary = "Project durable memory (KURISU.md):\n- Remember reconnect state.",
    ...
    public void OpenAiCompatibleProtocol_BuildPayload_UsesQwenSpecificToolCallGuidanceForDashScope()
    {
        ...
        var systemMessage = Assert.IsType<JsonObject>(messages[0]);
        var systemContent = systemMessage["content"]?.GetValue<string>() ?? string.Empty;
        Assert.Contains("kurisu-compatible function calling", systemContent, StringComparison.OrdinalIgnoreCase);
    ...
    {
        var workspaceRoot = Path.Combine(Path.GetTempPath(), $"kurisu-tool-events-{Guid.NewGuid():N}");
    ...
    {
        var workspaceRoot = Path.Combine(Path.GetTempPath(), $"kurisu-tool-error-loop-{Guid.NewGuid():N}");
        Directory.CreateDirectory(workspaceRoot);
    ...
    {
        var resolvedWorkspace = workspaceRoot ?? Path.Combine(Path.GetTempPath(), $"kurisu-prompt-{Guid.NewGuid():N}");
    ...
    private static KurisuRuntimeProfile CreateRuntimeProfile(string workspaceRoot)
    {
