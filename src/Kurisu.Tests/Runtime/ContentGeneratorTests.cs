...
public sealed class ContentGeneratorTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-content-json-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-content-embedding-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            Assert.NotNull(capturedRequest);
            Assert.Equal("https://api.openai.com/v1/embeddings", capturedRequest!.RequestUri!.ToString());
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-content-system-prompt-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
