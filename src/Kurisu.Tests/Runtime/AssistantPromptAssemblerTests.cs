...
public sealed class AssistantPromptAssemblerTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-context-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            File.WriteAllText(
                ...
                **Update time**: 2026-04-02T09:00:00Z
                ...
                ## Overall Goal
                Finish the native kurisu desktop runtime.
            ...
            var promptContext = await assembler.AssembleAsync(
                new AssistantTurnRequest
                {
                    ...
                    RuntimeProfile = new KurisuRuntimeProfile
                    {
            ...
            Assert.Equal("Finish the native kurisu desktop runtime.", promptContext.ProjectSummary.OverallGoal);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-context-untrusted-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-compressed-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-session-memory-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-budget-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
