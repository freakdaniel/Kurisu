...
public sealed class AssistantTurnRuntimeTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-tool-loop-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-loop-stop-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-approval-stop-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-approval-seed-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-multi-tool-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-tool-error-recovery-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-assistant-provider-error-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    private sealed class ThrowingAssistantResponseProvider : IAssistantResponseProvider
    {
        ...
        public Task<AssistantTurnResponse?> TryGenerateAsync(
            ...
            throw new AssistantProviderRequestException(
                Name,
                "https://portal.kurisu.ai/v1/chat/completions",
                401,
                "{\"error\":\"invalid_token\"}");
    }
