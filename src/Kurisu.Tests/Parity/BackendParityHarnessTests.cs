...
public sealed class BackendParityHarnessTests
{
    ...
    {
        var workspaceRoot = Path.Combine(Path.GetTempPath(), $"kurisu-parity-workspace-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-parity-telemetry-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-parity-auth-{authType}-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
        ...
        try
        {
            ...
            var authService = new AuthFlowService(runtimeProfileService, environmentPaths);
