...
public sealed class McpRegistryTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-registry-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-registry-override-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-registry-remove-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            var settingsContent = await File.ReadAllTextAsync(Path.Combine(homeRoot, ".kurisu", "settings.json"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-mcp-registry-auth-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
