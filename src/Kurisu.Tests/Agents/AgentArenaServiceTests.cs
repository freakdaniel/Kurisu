...
public sealed class AgentArenaServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-arena-{Guid.NewGuid():N}");
        ...
        {
            TryRunGit(workspaceRoot, "worktree", "remove", "--force", Path.Combine(homeRoot, ".kurisu", "worktrees", "arena-test-session", "worktrees", "Alpha"));
            TryRunGit(workspaceRoot, "worktree", "remove", "--force", Path.Combine(homeRoot, ".kurisu", "worktrees", "arena-test-session", "worktrees", "Beta"));
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-arena-cleanup-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
        var systemRoot = Path.Combine(root, "system");
        Directory.CreateDirectory(workspaceRoot);
        ...
        try
        {
            ...
            Assert.Equal("completed", result.Status);
            var sessionDirectory = Path.Combine(homeRoot, ".kurisu", "arena", "arena-cleanup-session");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-arena-followup-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
        var systemRoot = Path.Combine(root, "system");
        Directory.CreateDirectory(workspaceRoot);
        ...
        try
        {
            ...
            var sessionDirectory = Path.Combine(homeRoot, ".kurisu", "arena", "arena-round-session");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-arena-live-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
        var systemRoot = Path.Combine(root, "system");
        Directory.CreateDirectory(workspaceRoot);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-arena-cancel-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
        var systemRoot = Path.Combine(root, "system");
        Directory.CreateDirectory(workspaceRoot);
        ...
        try
        {
            ...
            var resultPath = Path.Combine(homeRoot, ".kurisu", "arena", "arena-cancel-session", "result.json");
            var statusPath = Path.Combine(homeRoot, ".kurisu", "arena", "arena-cancel-session", "status.json");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-arena-apply-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
        var systemRoot = Path.Combine(root, "system");
        Directory.CreateDirectory(workspaceRoot);
        ...
        try
        {
            ...
            var sessionDirectory = Path.Combine(homeRoot, ".kurisu", "arena", "arena-apply-session");
            var resultPath = Path.Combine(sessionDirectory, "result.json");
            var configPath = Path.Combine(sessionDirectory, "config.json");
            var statusPath = Path.Combine(sessionDirectory, "status.json");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-arena-discard-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
        var systemRoot = Path.Combine(root, "system");
        Directory.CreateDirectory(workspaceRoot);
        ...
        try
        {
            ...
            var sessionDirectory = Path.Combine(homeRoot, ".kurisu", "arena", "arena-discard-session");
            Assert.True(Directory.Exists(sessionDirectory));
            var resultPath = Path.Combine(sessionDirectory, "result.json");
            var statusPath = Path.Combine(sessionDirectory, "status.json");
