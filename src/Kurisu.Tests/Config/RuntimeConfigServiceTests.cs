...
public sealed class RuntimeConfigServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-config-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            var userQwenRoot = Path.Combine(homeRoot, ".kurisu");
            ...
            File.WriteAllText(
                ...
                """
                {
                  ...
                  "overrideExtensions": ["org.kurisu.desktop"],
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-config-untrusted-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-config-scope-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
