...
public sealed class SessionRecoveryTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-session-recovery-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-session-dismiss-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
