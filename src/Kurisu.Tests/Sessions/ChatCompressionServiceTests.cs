...
public sealed class ChatCompressionServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-chat-compression-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            var checkpoint = await service.TryCreateCheckpointAsync(
                new KurisuRuntimeProfile
                {
                    ProjectRoot = root,
                    GlobalKurisuDirectory = root,
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-chat-compression-disabled-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-chat-compression-llm-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
