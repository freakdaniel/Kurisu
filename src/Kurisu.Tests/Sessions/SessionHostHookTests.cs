...
public sealed class SessionHostHookTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-session-hook-block-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-session-hook-lifecycle-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
