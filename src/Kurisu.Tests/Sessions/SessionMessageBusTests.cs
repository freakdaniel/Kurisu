...
public sealed class SessionMessageBusTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-session-bus-tool-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-session-bus-question-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
