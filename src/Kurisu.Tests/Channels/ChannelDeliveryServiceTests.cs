...
public sealed class ChannelDeliveryServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-channel-outbox-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            var outboxPath = Path.Combine(homeRoot, ".kurisu", "channels", "outbox", "ops.jsonl");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-channel-send-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-channel-block-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
