...
public sealed class ChannelRuntimeParityTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-channel-group-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-channel-command-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-channel-collect-default-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-channel-attachments-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    private static (string WorkspaceRoot, ChannelRuntimeService Runtime) CreateRuntime(string root, string settingsJson)
    {
        ...
        Directory.CreateDirectory(homeRoot);
        Directory.CreateDirectory(systemRoot);
        File.WriteAllText(Path.Combine(workspaceRoot, ".kurisu", "settings.json"), settingsJson);
    ...
    private static SessionPreview CreateSessionPreview(StartDesktopSessionTurnRequest request) =>
        new()
        {
            ...
            GitBranch = "main",
            TranscriptPath = Path.Combine(request.WorkingDirectory, ".kurisu", "session.jsonl"),
            MetadataPath = Path.Combine(request.WorkingDirectory, ".kurisu", "session.meta.json")
        };
