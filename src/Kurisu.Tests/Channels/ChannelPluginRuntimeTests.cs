...
public sealed class ChannelPluginRuntimeTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-channel-plugin-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            var extensionRoot = Path.Combine(homeRoot, ".kurisu", "extensions", "plugin-example");
            Directory.CreateDirectory(extensionRoot);
            await File.WriteAllTextAsync(
                Path.Combine(extensionRoot, "kurisu-extension.json"),
                """
                {
                  "name": "plugin-example",
                  "version": "1.0.0",
