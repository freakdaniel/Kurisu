...
public sealed class ChannelRegistryServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-channels-{Guid.NewGuid():N}");
        ...
        try
        {
            ...
            var kurisuRoot = Path.Combine(homeRoot, ".kurisu");
            var channelsRoot = Path.Combine(kurisuRoot, "channels");
            Directory.CreateDirectory(kurisuRoot);
            Directory.CreateDirectory(channelsRoot);

            File.WriteAllText(
                Path.Combine(kurisuRoot, "settings.json"),
                """
                {
                  "channels": {
                    "team-telegram": {
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-pairings-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-approve-{Guid.NewGuid():N}");
        var workspaceRoot = Path.Combine(root, "workspace");
        var homeRoot = Path.Combine(root, "home");
