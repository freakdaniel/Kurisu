...
public sealed class UserPromptHookServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-hooks-modify-{Guid.NewGuid():N}");
        ...
        try
        {
                ...
                File.WriteAllText(
                    ...
                    $result = @{
                      ...
                      hookSpecificOutput = @{
                        ...
                        additionalContext = 'Use kurisu hook context'
                      }
                    }
            ...
            else
            {
                command = CrossPlatformTestSupport.CreateHookCommand(
                    ...
                    """
                    payload="$(cat)"
                    printf '%s' '{"decision":"allow","systemMessage":"Hook system note","hookSpecificOutput":{"modifiedPrompt":"Build the missing runtime layer. [hooked]","additionalContext":"Use kurisu hook context"}}'
                    """);
            }
            File.WriteAllText(
            ...
            var result = await service.ExecuteAsync(
                ...
                new UserPromptHookRequest
                {
                    ...
                    TranscriptPath = Path.Combine(workspaceRoot, ".kurisu", "chats", "session-1.jsonl")
                });

            Assert.False(result.IsBlocked);
            Assert.Equal("Build the missing runtime layer. [hooked]", result.EffectivePrompt);
            Assert.Equal("Use kurisu hook context", result.AdditionalContext);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-hooks-untrusted-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            var result = await service.ExecuteAsync(
                ...
                new UserPromptHookRequest
                {
                    ...
                    WorkingDirectory = workspaceRoot,
                    TranscriptPath = Path.Combine(workspaceRoot, ".kurisu", "chats", "session-2.jsonl")
                });
