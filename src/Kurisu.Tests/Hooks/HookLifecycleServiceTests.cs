...
public sealed class HookLifecycleServiceTests
{
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-hooks-pretool-{Guid.NewGuid():N}");
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-hooks-matcher-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"kurisu-hooks-extension-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        ...
        try
        {
            ...
            File.WriteAllText(
                Path.Combine(extensionSourceRoot, "kurisu-extension.json"),
                """
                {
                  "name": "hooked-ext",
                  "version": "1.0.0",
