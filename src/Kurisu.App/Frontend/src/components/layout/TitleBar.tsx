...
export default function TitleBar({ onToggleSidebar }: TitleBarProps) {
  ...
  const handleDragStart = (event: React.PointerEvent) => {
    ...
    window.kurisuDesktop?.beginWindowDrag?.();
  };

  const handleMinimize = () => {
    window.kurisuDesktop?.minimizeWindow?.();
  };

  const handleMaximize = () => {
    window.kurisuDesktop?.maximizeWindow?.();
  };
      ...
      <HStack spacing={2} flex={1} pl={3}>
        ...
        >
          <Box
            ...
            flexShrink={0}
          >
            <img src={kurisuLogo} alt="Qwen" style={{ width: '18px', height: '18px' }} />
          </Box>
          <Text
            fontSize="xs"
            fontWeight="medium"
