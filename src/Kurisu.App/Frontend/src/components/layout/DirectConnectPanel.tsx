...
export default function DirectConnectPanel({ isOpen, onClose }: DirectConnectPanelProps) {
  ...
  const refresh = async () => {
    ...
    try {
      const [nextServer, nextSessions] = await Promise.all([
        window.kurisuDesktop.getDirectConnectServer(),
        window.kurisuDesktop.listDirectConnectSessions(),
      ])
  ...
  const createSession = async () => {
    ...
    setError('')
    try {
      await window.kurisuDesktop.createDirectConnectSession({
        preferredSessionId: '',
        workingDirectory: '',
      })
      await refresh()
  ...
  const closeSession = async (directConnectSessionId: string) => {
    ...
    setError('')
    try {
      await window.kurisuDesktop.closeDirectConnectSession({ directConnectSessionId })
      await refresh()
    } catch (exception) {
      setError(exception instanceof Error ? exception.message : String(exception))
      setIsLoading(false)
