...
function useBootstrapState(): BootstrapState {
  ...
  const [authSnapshot, setAuthSnapshot] = useState<AuthStatusSnapshot>(fallbackBootstrap.kurisuAuth)
  const [mcpSnapshot, setMcpSnapshot] = useState<McpSnapshot>(fallbackBootstrap.kurisuMcp)
  ...
  const updateAuthSnapshot = (snapshot: AuthStatusSnapshot) => {
    ...
    setBootstrap((current) => ({ ...current, kurisuAuth: snapshot }))
  }
      ...
      const normalizedPayload: AppBootstrapPayload = {
        ...
        kurisuModels:
          'kurisuModels' in payload && payload.kurisuModels
            ? (payload.kurisuModels as AppBootstrapPayload['kurisuModels'])
            : fallbackBootstrap.kurisuModels,
      }

      setBootstrap(normalizedPayload)
      setAuthSnapshot(normalizedPayload.kurisuAuth)
      setMcpSnapshot(normalizedPayload.kurisuMcp)
