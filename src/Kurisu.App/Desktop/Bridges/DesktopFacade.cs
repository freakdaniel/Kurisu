...
public sealed class DesktopFacade(
    ...
    IPromptBridge promptProjectionService,
    IMcpResourceBridge mcpResourceProjectionService,
    IFollowupBridge followupProjectionService,
    IExtensionBridge extensionProjectionService,
    ISessionBridge sessionProjectionService,
    ProviderModelLister providerModelLister) : IDesktopSurface
{
    /// <summary>
    ...
    public async Task<ListProviderModelsResponse> ListProviderModelsAsync(ListProviderModelsRequest request)
    {
        ...
        if (preset is null)
        {
            ...
            {
                Models = [],
                Error = $"Unknown provider preset '{request.PresetId}'.",
                FromCache = false
            };
        }

        var result = await providerModelLister.ListAsync(
            preset,
            request.ApiKey,
            forceRefresh: request.ForceRefresh).ConfigureAwait(false);
