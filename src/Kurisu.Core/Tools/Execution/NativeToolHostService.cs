    ...
    IApprovalSessionRuleStore? approvalSessionRuleStore = null) : IToolExecutor
{
    ...
    private readonly ISkillToolService skills = skillToolService ?? new SkillToolService(new KurisuCompatibilityService(new DefaultDesktopEnvironmentPaths()));
        ...
        string approvalState)
    {
        ...
        try
        {
            ...
            var output = recurring
                ? $"Scheduled recurring job {job.Id} ({job.CronExpression}). Session-only (not written to disk, dies when Kurisu Code exits). Auto-expires after 3 days. Use CronDelete to cancel sooner."
                : $"Scheduled one-shot task {job.Id} ({job.CronExpression}). Session-only (not written to disk, dies when Kurisu Code exits). It will fire once then auto-delete.";
