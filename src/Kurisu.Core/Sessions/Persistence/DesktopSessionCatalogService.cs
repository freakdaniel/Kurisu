    ...
    SessionsDb? sessionsDb = null) : ITranscriptStore, ISessionService
{
    ...
    private string ResolveTranscriptPath(WorkspacePaths paths, KurisuRuntimeProfile runtimeProfile, string sessionId)
    {
    ...
    private static string FindGlobalTranscriptPath(KurisuRuntimeProfile runtimeProfile, string sessionId)
    {
        if (string.IsNullOrWhiteSpace(sessionId) ||
            string.IsNullOrWhiteSpace(runtimeProfile.GlobalKurisuDirectory))
        {
            return string.Empty;
        }

        var projectsDirectory = Path.Combine(runtimeProfile.GlobalKurisuDirectory, "projects");
    ...
    private static string? TryExtractFullText(JsonElement root)
    {
        ...
        // Google/Kurisu format: message.parts[].text  (skip parts with "thought": true)
        if (TryGetProperty(message, "parts", out var parts) &&
            parts.ValueKind == JsonValueKind.Array)
        {
    ...
    private static string? TryExtractThinkingText(JsonElement root)
    {
        ...
        // Google/Kurisu format: parts with "thought": true
        if (TryGetProperty(message, "parts", out var parts) &&
            parts.ValueKind == JsonValueKind.Array)
        {
            var sb = new System.Text.StringBuilder();
