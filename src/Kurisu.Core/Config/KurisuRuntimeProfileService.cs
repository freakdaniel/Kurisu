    ...
    RuntimeSelectionStore selectionStore)
{
    ...
    public KurisuRuntimeProfile Inspect(WorkspacePaths paths)
    {
        var snapshot = config.Inspect(paths);
        var runtimeSource = !string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("KURISU_RUNTIME_DIR"))
            ? "environment"
        ...
        return new KurisuRuntimeProfile
        {
            ...
            GlobalKurisuDirectory = snapshot.GlobalKurisuDirectory,
    ...
    {
        var runtimeDirectory = Environment.GetEnvironmentVariable("KURISU_RUNTIME_DIR");
        ...
        bool folderTrustEnabled)
    {
        ...
        var trustedFoldersPath = Environment.GetEnvironmentVariable("KURISU_TRUSTED_FOLDERS_PATH");
        if (string.IsNullOrWhiteSpace(trustedFoldersPath))
        {
            trustedFoldersPath = Path.Combine(globalKurisuDirectory, "trustedFolders.json");
        }
