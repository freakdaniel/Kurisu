    ...
    IConfigService? configService = null) : IChannelRegistryService
{
    ...
    public ChannelDefinition GetChannel(WorkspacePaths workspace, string name) =>
        ...
        ?? throw new InvalidOperationException($"Channel \"{name}\" was not found in merged kurisu settings.");
    ...
    private void EnsureChannelExists(KurisuRuntimeProfile runtime, WorkspacePaths workspace, string name)
    {
    ...
    private IReadOnlyList<ConfiguredChannel> ReadConfiguredChannels(KurisuRuntimeProfile runtime, WorkspacePaths workspace)
    {
