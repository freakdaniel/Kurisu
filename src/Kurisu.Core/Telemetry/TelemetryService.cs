...
namespace Kurisu.Core.Telemetry;
...
public sealed class TelemetryService(ILogger<TelemetryService> logger) : ITelemetryService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await IncrementAsync(
            runtimeProfile,
            "kurisu.subagent.execution.count",
            1,
            tags: new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                ["agent"] = agentName,
    ...
    private static bool IsEnabled(KurisuRuntimeProfile runtimeProfile)
    {
    ...
    private static JsonObject SanitizePayload(KurisuRuntimeProfile runtimeProfile, JsonObject payload)
    {
    ...
    private static (string EventsPath, string MetricsPath) ResolvePaths(KurisuRuntimeProfile runtimeProfile)
    {
    ...
    {
        if (!metricName.StartsWith("kurisu.", StringComparison.OrdinalIgnoreCase))
        {
            return [metricName];
        }

        return [metricName, metricName["kurisu.".Length..]];
    }
}
