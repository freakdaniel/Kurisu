...
/// <summary>
/// Represents the Kurisu Compatibility Service
/// </summary>
...
public sealed partial class KurisuCompatibilityService(IDesktopEnvironmentPaths environmentPaths)
{
    ...
    public KurisuCompatibilitySnapshot Inspect(WorkspacePaths paths)
    {
        ...
        return new KurisuCompatibilitySnapshot
        {
            ...
            [
                CreateSurfaceDirectory(
                    "project-commands",
                    "Project commands",
                    Path.Combine(projectKurisuRoot, "commands"),
                    "Slash-command markdown and command surfaces"),
                CreateSurfaceDirectory(
                    "project-skills",
                    "Project skills",
                    Path.Combine(projectKurisuRoot, "skills"),
                    "Project-local skills stored as directories with SKILL.md"),
                CreateSurfaceDirectory(
                    "user-skills",
                    "User skills",
                    Path.Combine(homeKurisuRoot, "skills"),
            ...
            Commands =
            [
                .. DiscoverCommands(Path.Combine(homeKurisuRoot, "commands"), "user"),
                .. (includeProjectSurfaces
                    ? DiscoverCommands(Path.Combine(projectKurisuRoot, "commands"), "project")
                    : [])
            ],
            Skills =
            [
                .. DiscoverSkills(Path.Combine(homeKurisuRoot, "skills"), "user"),
                .. (includeProjectSurfaces
                    ? DiscoverSkills(Path.Combine(projectKurisuRoot, "skills"), "project")
                    : [])
            ]
        };
    }

    private static KurisuCompatibilityLayer CreateLayer(
        string id,
        string title,
        string scope,
        int priority,
        string path)
    {
        ...
        return new KurisuCompatibilityLayer
        {
            Id = id,
            Title = title,
            Scope = scope,
    ...
    private static KurisuSurfaceDirectory CreateSurfaceDirectory(
        string id,
        string title,
        string path,
        string summary)
    {
        ...
        return new KurisuSurfaceDirectory
        {
            Id = id,
            Title = title,
            Path = path,
    ...
    private static IReadOnlyList<KurisuCommandSurface> DiscoverCommands(string rootPath, string scope)
    {
        ...
        return Directory.EnumerateFiles(rootPath, "*.md", SearchOption.AllDirectories)
            .Select(path =>
            {
                ...
                return new KurisuCommandSurface
                {
    ...
    private static IReadOnlyList<KurisuSkillSurface> DiscoverSkills(string rootPath, string scope)
    {
        if (!Directory.Exists(rootPath))
        {
            return [];
