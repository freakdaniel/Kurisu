...
""";
    ...
    private static IReadOnlyList<string> ReadContextFiles(KurisuRuntimeProfile runtimeProfile, string workingDirectory)
    {
        ...
        foreach (var path in paths.Take(MaxContextFiles))
        {
            try
            {
                ...
                var displayPath = GetDisplayPath(runtimeProfile.ProjectRoot, runtimeProfile.GlobalKurisuDirectory, path);
                results.Add(
    ...
    private static IReadOnlyList<string> DiscoverContextFilePaths(KurisuRuntimeProfile runtimeProfile, string workingDirectory)
    {
        ...
        {
            var globalPath = Path.Combine(runtimeProfile.GlobalKurisuDirectory, fileName);
            TryAdd(globalPath);
        }
    ...
    private static string GetDisplayPath(string projectRoot, string globalKurisuDirectory, string path)
    {
        var fullPath = Path.GetFullPath(path);
        var normalizedProjectRoot = Path.GetFullPath(projectRoot);
        var normalizedGlobalDirectory = Path.GetFullPath(globalKurisuDirectory);
        ...
        if (fullPath.StartsWith(
                ...
                OperatingSystem.IsWindows() ? StringComparison.OrdinalIgnoreCase : StringComparison.Ordinal))
        {
            return Path.Combine("~", ".kurisu", Path.GetRelativePath(normalizedGlobalDirectory, fullPath));
        }
...
""";
    ...
    private static string BuildDurableMemorySummary(KurisuRuntimeProfile runtimeProfile)
    {
        var sections = new List<string>();
        sections.AddRange(BuildDurableMemoryScopeSections("Global", runtimeProfile.GlobalKurisuDirectory, runtimeProfile.ContextFileNames));
        ...
        string scope)
    {
        ...
        {
            var isGlobalInstruction = IsPathWithin(path, runtimeProfile.GlobalKurisuDirectory);
            ...
            try
            {
                ...
                sections.Add(
                    $$"""
From {{GetDisplayPath(runtimeProfile.ProjectRoot, runtimeProfile.GlobalKurisuDirectory, path)}}:
