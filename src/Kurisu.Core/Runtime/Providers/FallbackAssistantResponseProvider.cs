        ...
        IReadOnlyList<AssistantToolCallResult> toolHistory)
    {
        if (toolHistory.Count > 0)
        {
            ...
            return lastTool.Execution.Status switch
            {
                ...
                ToolExecutionStatus.ApprovalRequired => $"Tool '{lastTool.Execution.ToolName}' is waiting for approval before the turn can continue.",
        ...
        {
            if (string.Equals(request.ToolExecution.ToolName, WellKnownToolNames.AskUserQuestion, StringComparison.OrdinalIgnoreCase) &&
        ...
        if (request.ResolvedCommand is not null)
        {
            return request.ToolExecution.Status switch
            {
                ...
                ToolExecutionStatus.ApprovalRequired => $"Slash command '/{request.ResolvedCommand.Name}' resolved and tool '{request.ToolExecution.ToolName}' is waiting for approval.",
        ...
        return request.ToolExecution.Status switch
        {
            ...
            ToolExecutionStatus.ApprovalRequired => $"Tool '{request.ToolExecution.ToolName}' is waiting for approval before execution.",
