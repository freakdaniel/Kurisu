...
public sealed class FileDiscoveryService(
    ...
    KurisuRuntimeProfileService runtimeProfileService) : IFileDiscoveryService
{
    ...
    public FileDiscoverySnapshot Inspect(WorkspacePaths paths)
    {
        ...
        var kurisuIgnorePath = Path.Combine(runtimeProfile.ProjectRoot, ".kurisuignore");
        var kurisuIgnoreRules = LoadIgnoreRules(kurisuIgnorePath);
        ...
        var kurisuIgnoredFiles = candidateFiles
            .Where(path => ShouldIgnore(path, kurisuIgnoreRules))
            .ToArray();
        var visibleFiles = candidateFiles
            .Except(kurisuIgnoredFiles, StringComparer.OrdinalIgnoreCase)
    ...
    private static IReadOnlyList<IgnoreRule> LoadIgnoreRules(string kurisuIgnorePath)
    {
        if (!File.Exists(kurisuIgnorePath))
        {
            return [];
        }

            return File.ReadAllLines(kurisuIgnorePath)
