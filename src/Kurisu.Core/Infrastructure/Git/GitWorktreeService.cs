    ...
    IGitHistoryService? gitHistoryService = null) : IGitWorktreeService
{
    ...
    public GitRepositorySnapshot CreateManagedWorktree(WorkspacePaths paths, CreateManagedWorktreeRequest request)
    {
        ...
        var sessionDirectory = Path.Combine(runtimeProfile.GlobalKurisuDirectory, "worktrees", sessionId);
    ...
    {
        var runtimeProfile = runtimeProfileService.Inspect(paths);
        var managedWorktreesRoot = Path.Combine(runtimeProfile.GlobalKurisuDirectory, "worktrees");
